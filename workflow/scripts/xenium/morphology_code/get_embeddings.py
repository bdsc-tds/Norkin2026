...
def plot_organoids(joined_df, patient_id=""):
    joined_df.plot(
    ...
    for organoid_id, group in joined_df.groupby("Rel_Organoid_ID"):
        ...
        # Draw bounding box
        rect = plt.Rectangle(
            (minx, miny), maxx - minx, maxy - miny, fill=False, edgecolor="red", linewidth=1.5, linestyle="--"
        )
        ax.add_patch(rect)
...
def visualize_cell_segmentation(parquet_path, limit=None):
    ...
    poly_collection = PolyCollection(polygons, facecolors=colors, edgecolors="black", linewidths=0.5, alpha=0.7)
    ...
    sm.set_array([])
    _ = plt.colorbar(sm, ax=ax, label="Cell ID")

    plt.show()


def create_organoid_regions(
    parquet_path, buffer_distance=5, min_cell_count=20, plot_results=True, outline_thickness=1.0
):
    """
...
def generate_organoid_masks_with_square_bboxes(
    ...
    for organoid_id in organoid_ids:
        ...
        if not scale:
            ...
            bbox_with_padding = (min_x - padding_x, min_y - padding_y, max_x + padding_x, max_y + padding_y)
        ...
        def draw_thick_line(mask, r0, c0, r1, c1, thickness):
            if thickness == 1:
                ...
                for t in range(-thickness // 2, thickness // 2 + 1):
                    ...
                    mask[rr[valid], cc[valid]] = np.maximum(
                        mask[rr[valid], cc[valid]], (val[valid] * 255).astype(np.uint8)
                    )
...
def polygon_to_mask(
    polygons, scale=True, output_size=(224, 224), padding_ratio=0.1, outline_thickness=1, max_pixel_side_length=None
):
    """
    ...
    def draw_thick_line(mask, r0, c0, r1, c1, thickness):
        if thickness == 1:
            ...
            for t in range(-thickness // 2, thickness // 2 + 1):
                rr, cc, val = line_aa(r0 + t, c0 + t, r1 + t, c1 + t)
                valid = (rr >= 0) & (rr < mask.shape[0]) & (cc >= 0) & (cc < mask.shape[1])
                mask[rr[valid], cc[valid]] = np.maximum(mask[rr[valid], cc[valid]], (val[valid] * 255).astype(np.uint8))
...
class NorkinOrganoidDataset(torch.utils.data.Dataset):
    ...
    def _process_raw_data_from_parquet(self):
        ...
        all_cell_boundary_parquet_files = glob.glob(
            os.path.join(self.raw_data_path, "**", "cell_boundaries.parquet"), recursive=True
        )
        ...
        for cell_boundary_pth in tqdm(all_cell_boundary_parquet_files, desc="Generating organoid polygons..."):
            ...
            def all_dims(boundary_boxes):
                return [dims(b) for b in boundary_boxes]

            def max_dim(boundary_boxes):
                return np.max(np.array(all_dims(boundary_boxes)), axis=0).max()
    ...
    def __getitem__(self, idx):
        if isinstance(idx, slice):
            ...
            tensors = [
                torch.FloatTensor(self.organoid_masks[list(self.organoid_masks.keys())[i]]).unsqueeze(0) for i in idx
            ]
            return torch.cat(tensors, dim=0)
        else:
...
def get_resnet152_embeddings(
    ...
    if fine_tune and morphological_features is not None:
        ...
        train_dataset, val_dataset = torch.utils.data.random_split(
            dataset, [1 - validation_split, validation_split], generator=torch.Generator().manual_seed(42)
        )
        ...
        mlp_head = nn.Sequential(nn.Linear(2048, 1024), nn.ReLU(), nn.Linear(1024, feature_dim)).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            if early_stopping_patience is not None:
                if val_loss < best_val_loss:
                    ...
                    best_model_state = {"resnet": resnet_model.state_dict(), "mlp_head": mlp_head.state_dict()}
                else:
